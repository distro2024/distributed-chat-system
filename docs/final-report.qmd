...
### 1. Vector Clocks for Causal Relationships
...
### 2. Priority Queue for Message Ordering
- Messages are added to a priority queue, sorted by vector clocks.
- **Concurrent messages** (with equal clocks) are resolved deterministically using timestamps as tiebreakers.

### 3. Consistency Across Nodes
- All nodes apply the same logic to ensure consistent message ordering across the system, regardless of the delivery sequence.
- Duplicate messages are ignored, and counters are not incremented during reception.

## Handling Message Loss
**TODO:** Add details about message loss handling strategies to guarantee that all nodes share a common state.

## Summary
This implementation guarantees **causal consistency** across the system when all messages are successfully delivered. By combining vector clocks and priority queues, the system ensures reliable and ordered communication in a distributed environment.
